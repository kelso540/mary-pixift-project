...
// NOTES: Added count to make prev and next work
let count = 0; 
...
window.onload = function() {
  ...
  // NOTES: Added a few changes to get this to work 
  const up = ()=>{  
    const newImg = document.querySelector(".current-photo-image");

    /* NOTES: using Number() converts any value into a number if possible so this could be used above 
    on photos.filter currentId but == does the same thing. */
